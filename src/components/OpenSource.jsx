...
const OpenSource = () => {
  ...
  useEffect(() => {
    const getContributions = async () => {
      ...
      console.log("What is fetchedContributions?", fetchedContributions);
      console.log("Is it an array?", Array.isArray(fetchedContributions));
      console.log("Type:", typeof fetchedContributions);

      // SAFETY CHECK: Make sure it's an array
      const contributionsArray = Array.isArray(fetchedContributions)
        ? fetchedContributions
        : [];

      setContributions(contributionsArray);
      setFilterContribution(contributionsArray);

      // Filters based on fetched contributions
      if (contributionsArray.length > 0 && !contributionsArray.error) {
        const uniqueRepos = [...new Set(contributionsArray.map(c => c.repo))];
